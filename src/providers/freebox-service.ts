...
export class FreeboxService {
    ...
    constructor(private http: HttpClient, public commonService: CommonService,
        ...
        this.routeApi = this.configService.urlApiFreebox;
    ...
    auth() {
        return new Promise(resolve => {
            let request: any = {
                ...
                "app_name": this.configService.appName,
                "app_version": this.configService.appVersion,
                "device_name": this.configService.appDevice
            };
            ...
            this.http.post(this.routeAuth, param)
                .subscribe(
                    response => {
                        if (response['success']) {
                            this.commonService.setElement('token', response['result']['app_token']).then(setToken => {
                                if (setToken) {
                                    this.commonService.setElement('trackId', response['result']['track_id']).then(setTrackId => {
                                        if (setTrackId) {
                                            resolve(true);
                                        } else {
                                            resolve(false);
    ...
    getStatus() {
        return new Promise(resolve => {
            this.commonService.getElement('trackId').then(trackId => {
                if (!trackId) {
                        ...
                        .subscribe(
                            response => {
                                if (response['success']) {
                                    ...
                                    if (status == 'granted') {
                                        this.commonService.setElement('granted', true).then(granted => {
                                            if (granted) {
                                                resolve(status);
                                            } else {
                                                resolve('errorSet');
                ...
                .subscribe(
                    response => {
                        if (response['success']) {
                            ...
                            this.commonService.getElement('token').then(token => {
        ...
        return new Promise(resolve => {
            ...
            this.http.post(this.routeLoginSession, param, reqOpts)
                .subscribe(
                    response => {
                        if (response['success']) {
                            this.commonService.setElement('tokenSession', response['result']['session_token']).then(set => {
                                resolve(response['result']['session_token']);
                            });

                        } else {
                            this.commonService.removeElement('tokenSession').then(() => {
                                resolve(false);
                            });
                        }
                    },
    ...
    launch(method=null, parameters=[]) {
        return new Promise(resolve => {
            if (method) {
                this.commonService.getElement('tokenSession').then(tokenSession => {
                    if (tokenSession) {
